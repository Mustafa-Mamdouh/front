...
export class ServiceCategoryComponent implements OnInit {
    servicename: string = "ServiceCategory";
    components = new Array<IServiceCategory>();
    public toBeSaved = new IServiceCategory();
    toBeDeleted: number;
        ...
        this.lookupService.getAllObjects(this.servicename).subscribe(
            (data: any) => {
                if (data.messageResponseObj.code == "000") {
                    //Succed
                    this.components = data.categories;
                }
                else {
                    //Fail
                    alert("fail");
        ...
        this.lookupService.addObject(this.toBeSaved, this.servicename).subscribe(
            (data: any) => {
                if (data.messageResponseObj.code == "000") {
                    //Succed
                    //add object to view
                    this.toBeSaved.id = data.serviceCategory.id;
                    this.components.push(this.toBeSaved);
                    //reintialize the temp oject
                    this.toBeSaved = new IServiceCategory();
                    this.closeModal();
                } else {
                    //Fail
                    alert("fail");
