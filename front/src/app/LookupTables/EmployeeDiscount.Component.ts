...
import { IEmployeeDiscount } from '../Infrastructure/IEmployeeDiscount';
import { NgForm, FormBuilder, Validators } from "@angular/forms";

@Component({
    moduleId: module.id,
    selector: 'EmployeeDiscount',
    templateUrl: './EmployeeDiscount.html',
    styleUrls: ['./Tabble.css']
})
export class EmployeeDiscountComponent implements OnInit {
    servicename: string = "EmployeeDiscount";
    components = new Array<IEmployeeDiscount>();
    public toBeSaved = new IEmployeeDiscount();
    toBeDeleted: number;
        ...
        this.lookupService.getAllObjects(this.servicename).subscribe(
            (data: any) => {
                if (data.messageResponseObj.code == "000") {
                    //Succed
                    this.components = data.discounts;
                }
                else {
                    //Fail
                    alert("fail");
        ...
        this.lookupService.addObject(this.toBeSaved, this.servicename).subscribe(
            (data: any) => {
                if (data.messageResponseObj.code == "000") {
                    //Succed
                    //add object to view
                    this.toBeSaved.id = data.employeeDiscountCategory.id;
                    this.components.push(this.toBeSaved);
                    //reintialize the temp oject
                    this.toBeSaved = new IEmployeeDiscount();
                    this.closeModal();
                } else {
                    //Fail
                    alert("fail");
